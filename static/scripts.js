...
document
  .getElementById("GeneratenNewDataset")
...
function updateButtonStates() {
  ...
  if (init_method !== "manual") {
    ...
    document.getElementById("runKMeans").disabled =
      selected_centroids.length !== num_clusters;
    document.getElementById("runConvergence").disabled =
      selected_centroids.length !== num_clusters;
  }
}
    ...
    .then((data) => {
      ...
      function updateKMeans() {
        if (i < centroids.length) {
          ...
          Plotly.newPlot(
            "graph",
            scatterData.concat([centroidScatter]),
            defaultLayout()
          );
...
function setupClickListener() {
  ...
  clickHandler = function (event) {
    if (selected_centroids.length < num_clusters) {
      ...
      if (
        xInDataCoords >= xaxis.range[0] &&
        xInDataCoords <= xaxis.range[1] &&
        yInDataCoords >= yaxis.range[0] &&
        yInDataCoords <= yaxis.range[1]
      ) {
...
document.getElementById("reset").addEventListener("click", function () {
  ...
  for (let i = 0; i < traceCount; i++) {
    if (
      graphDiv.data[i].name === "Centroids" ||
      (graphDiv.data[i].marker && graphDiv.data[i].marker.symbol === "cross")
    ) {
