...
def kmeans_plus_plus_initialization(X, n_clusters):
    ...
    for _ in range(1, n_clusters):
        ...
        next_centroid_idx = np.searchsorted(cumulative_probabilities, np.random.rand())
...
def kmeans_with_iterations(data, n_clusters, init_method, run_to_convergence=False):
    ...
    centroids_list, labels_list = [centroids.copy()], []
    ...
    for _ in range(100):
        ...
        for i in range(n_clusters):
            ...
            new_centroids.append(points_in_cluster.mean(axis=0) if len(points_in_cluster) > 0 else centroids[i])
        ...
        if run_to_convergence:
            centroids_list, labels_list = [new_centroids], [labels]
            break
